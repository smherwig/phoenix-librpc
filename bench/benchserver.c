...
static uint8_t *bench_download_data = NULL;
 ...
 * server responds with non-emtpy body
 */
static void
bench_download_proxy(struct bench_client *client)
{
    ...
    rho_buf_write(buf, bench_download_data, bench_download_size);
...
main(int argc, char *argv[])
{
    ...
    bench_download_data = rhoL_zalloc(bench_download_size);
    ...
    if (bench_download_data != NULL)
        rhoL_free(bench_download_data);
